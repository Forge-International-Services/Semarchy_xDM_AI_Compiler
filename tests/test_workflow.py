...
@pytest.mark.parametrize("kind", ["Router", "ParallelBlock"])
def test_an_unattested_step_type_is_refused(kind):
    w = _wf(steps=[WorkflowStep(type="StartEvent", name="start",
                                transitions=[Transition(to_step="x")]),
                   WorkflowStep(type=kind, name="x",
                                transitions=[Transition(to_step="done")]),
                   WorkflowStep(type="EndEvent", name="done")])
    with pytest.raises(WorkflowEmitError, match="no attested shape"):
        definition(w)
...
def test_a_submit_data_without_a_job_is_refused():
    w = _wf(steps=[WorkflowStep(type="Automation", name="a",
                                automation_type="SUBMIT_DATA")])
    with pytest.raises(WorkflowEmitError, match="integration job"):
        definition(w)
...
def test_the_unattested_wire_spellings_are_labelled():
    """Only EMAIL has been seen in an export. The other four are named for the fields
    the builder exposes, which is a guess — recorded in one place with a label rather
    than spread through the emitter and forgotten."""
    from agent.compile.emit_workflow import UNATTESTED_WIRE_TYPES, WIRE_TYPE
    assert "EMAIL" not in UNATTESTED_WIRE_TYPES
    assert UNATTESTED_WIRE_TYPES == set(WIRE_TYPE) - {"EMAIL"}
